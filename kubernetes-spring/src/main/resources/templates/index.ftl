...
<h1 class="f2 mb0"><span class="gold">W</span>note</h1>
...
<form action="/note" method="POST" enctype="multipart/form-data">
    <ol class="list pl0">
        ...
        <li class="mv3">
            <label class="f6 b db mb2" for="description">Writing file name</label>
            <input class="f6 link dim br1 ba bw1 ph3 pv2 mb2 dib black bg-white pointer" type="text" value=""
                   placeholder="file name" required name="filename"><#if filename??>${filename}</#if></input>
            <label class="f6 b db mb2" for="description">Write your content here</label>
...
<ul class="list pl0"><p class="f6 b db mb2">Notes</p>
    <#if notes??>
        <#list notes as note>
            <li class="mv3 bb bw2 b--light-yellow bg-washed-yellow ph4"><p class="measure"></p>
                <p>Id: ${note.getFileId()}</p>
                <p>Name: <#if note.fileName??>${note.fileName}</#if></p>
                <p>${note.getDescription()}</p>
                <p>Last Modified:  <#if note.dateUpdate??>${note.dateUpdate}<#else>${note.date}</#if></p>
                <p></p>
            </li>
        <#else>
